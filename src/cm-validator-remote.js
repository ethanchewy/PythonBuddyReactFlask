//window.CodeMirror.remoteValidator = function(cm, updateLinting, options) {
export default function remoteValidator(cm, updateLinting, options) {
	...
	function result_cb(error_list)
	{
		...
		for(var i in error_list)
		{
			...
			found.push({
				from: window.CodeMirror.Pos(start_line - 1, start_char),
				to: window.CodeMirror.Pos(end_line - 1, end_char),
				message: message,
